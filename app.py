...
def generar_pdf_graficas(alumno_up, alumno_down=None):
    ...
    def dibujar_tarjeta(y_offset, datos):
        ...
        if os.path.exists(ruta_banner):
            c.drawImage(ruta_banner, 0.5 * inch, y_offset + 3.8 * inch, width=7.5 * inch, height=1.2 * inch, mask='auto')
        ...
        if foto_raw:
            try:
                ...
                c.drawImage(ImageReader(img_buf), 6.3 * inch, y_offset + 2.05 * inch, width=1.1 * inch, height=1.3 * inch)
            except Exception: pass

        c.setFillColor(colors.black)
        y_text = y_offset + 3.2 * inch
        for etiqueta, valor in [("COMPETIDOR:", datos['nombre_completo']), ("ESCUELA:", datos['escuela']), ("PROFESOR:", datos['profesor'])]:
            c.setFont("Helvetica-Bold", 14); c.drawString(0.8 * inch, y_text, etiqueta)
            c.setFont("Helvetica", 14); c.drawString(2.3 * inch, y_text, str(valor))
            c.line(2.2 * inch, y_text - 4, 5.9 * inch, y_text - 4)
            y_text -= 0.6 * inch

        c.setFont("Helvetica-Bold", 13); c.drawString(0.8 * inch, y_offset + 1.55 * inch, "EDAD:")
        c.setFont("Helvetica", 13); c.drawString(1.65 * inch, y_offset + 1.55 * inch, str(datos['edad']))
        ...
        c.setFillColor(colors.HexColor("#B22222")); c.drawString(6.65 * inch, y_offset + 1.55 * inch, str(datos.get('modalidad', '')))
        c.setFillColor(colors.black)

        c.setStrokeColor(colors.HexColor("#AAAAAA")); c.setLineWidth(0.5); c.line(0.65 * inch, y_offset + 1.3 * inch, 7.85 * inch, y_offset + 1.3 * inch)
        c.setFillColor(colors.HexColor("#F5F5F5")); c.roundRect(0.65 * inch, y_offset + 0.18 * inch, 7.2 * inch, 1.05 * inch, 4, fill=1, stroke=1)
        c.setFillColor(colors.HexColor("#1a1a2e")); c.setFont("Helvetica-Bold", 8); c.drawString(0.85 * inch, y_offset + 1.08 * inch, "DECLARACIONES Y ACEPTACIÓN DE RIESGOS")
        
        nombre_comp = datos['nombre_completo']
        lineas = [f"Yo, en mi calidad de padre, madre o tutor del competidor {nombre_comp}, declaro estar", "consciente de que el taekwondo es un deporte de contacto, por lo que autorizo la participación", "de mi hijo(a) en el torneo, después de haber leído y autorizado la declaración y aceptación de", 'riesgo en el registro al "4to Torneo Regional la Perla del Sur".']
        c.setFont("Helvetica", 7.5); c.setFillColor(colors.HexColor("#333333"))
        y_d = y_offset + 0.88 * inch
        for l in lineas: c.drawString(0.85 * inch, y_d, l); y_d -= 0.165 * inch

    dibujar_tarjeta(5.5 * inch, alumno_up)
    if alumno_down: dibujar_tarjeta(0.25 * inch, alumno_down)
...
with tab1:
    ...
    with col2:
        ...
        foto_upload = st.file_uploader("Sube una imagen", type=["jpg", "jpeg", "png"], key="foto_upload", label_visibility="collapsed")
        if foto_upload: st.image(foto_upload, caption="Vista previa", width=160)

    st.divider()
    st.markdown("""<div style='background:linear-gradient(135deg,#1a1a2e 0%,#16213e 100%);border:1px solid #c9a84c;border-radius:10px;padding:18px;color:#e8e8e8;font-size:13.5px;'><p>Yo, en mi calidad de <b style='color:#f0d080;'>padre, madre o tutor legal</b> del competidor registrado, autorizo su participación y asumo responsabilidad por los riesgos inherentes al deporte.</p></div>""", unsafe_allow_html=True)
    st.checkbox('✅ Acepto los términos y autorizo la participación.', key='acepta_riesgos')
    st.button('💾 GUARDAR REGISTRO', on_click=registrar_competidor, use_container_width=True, disabled=not st.session_state.get('acepta_riesgos', False))
        ...
        if not df.empty:
            ...
            with st.expander("📥 EXPORTAR DATOS PARA MIGRACIÓN"):
                ...
                col_exp2.download_button(
                    ...
                    data=json_data,
                    file_name='respaldo_competidores_tkd.json',
            ...
            cols_ocultar = ('id', 'foto', 'formas', 'combate')
            ...
            with col_acc1:
                ...
                st.download_button("🚀 DESCARGAR PDF", data=pdf, file_name="graficas.pdf", use_container_width=True)
            ...
            with col_acc2:
                ...
                with st.expander("✏️ EDITAR DATOS"):
                    ...
                    e_cin = col_e2.selectbox("Nueva Cinta", CINTAS, index=CINTAS.index(d['cinta']) if d['cinta'] in CINTAS else 0)
                    e_mod = st.selectbox('Modalidad', MODALIDADES, index=MODALIDADES.index(d['modalidad']) if d.get('modalidad') in MODALIDADES else 0)
                    
                    nueva_foto = st.file_uploader('Cambiar foto (opcional)', type=['jpg', 'jpeg', 'png'], key='edit_foto')
                    nueva_foto_bytes = nueva_foto.read() if nueva_foto else None

                    if st.button('💾 ACTUALIZAR DATOS', use_container_width=True):
                        actualizar_competidor(int(d['id']), e_nom, e_esc, e_pro, e_eda, e_cin, e_mod, nueva_foto_bytes)
                ...
                if st.button("🔄 ACTUALIZAR LISTA", use_container_width=True): st.rerun()
        else:
